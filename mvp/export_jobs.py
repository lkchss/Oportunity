"""Export the JOBS list to a single Obsidian tree (projects/lists/jobs).

Reuses note/dashboard/slug machinery from export_obsidian.py. Notes tagged #job so the Jobs
dashboard's Dataview queries never mix with the other lists. Same grade subfolders +
seen/review triage bins; add-only never overwrites your sorting.

Usage:
    python mvp/export_jobs.py --vault "C:/.../projects/lists" --subfolder jobs --by-grade --add-only
"""
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Export the jobs list to Obsidian Markdown.")
    ap.add_argument("--vault", help="path to the Obsidian vault root")
    ap.add_argument("--subfolder", default="jobs", help="folder inside the vault (or output dir)")
    ap.add_argument("--out", default=str(ROOT / "jobs_export"), help="output dir when --vault is omitted")
    ...
    for title, url, summary, why, category, found_at, grade, deadline in rows:
        ...
        (target / fname).write_text(
            note(title, grade, deadline, category, url, summary, why, kind="job"),
            encoding="utf-8",
        )
        written += 1

    dash = base / "_Jobs Dashboard.md"
    if not (args.add_only and dash.exists()):
        dash.write_text(dashboard("job", "Jobs"), encoding="utf-8")
