"""Export the HORIZONS list to a single Obsidian tree (e.g. projects/lists/horizons).

Reuses note/dashboard/slug machinery from export_obsidian.py, but writes one
flat tree (no positions split). Notes are tagged #horizon so the Horizons
dashboard's Dataview queries never mix with the career list. Same grade
subfolders + seen/review triage bins; add-only never overwrites your sorting.

Usage:
    python mvp/export_horizons.py --vault "C:/.../projects/lists" --subfolder horizons --by-grade --add-only
"""
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Export the horizons list to Obsidian Markdown.")
    ap.add_argument("--vault", help="path to the Obsidian vault root")
    ap.add_argument("--subfolder", default="horizons", help="folder inside the vault (or output dir)")
    ap.add_argument("--out", default=str(ROOT / "horizons_export"), help="output dir when --vault is omitted")
    ...
    for title, url, summary, why, category, found_at, grade, deadline in rows:
        ...
        (target / fname).write_text(
            note(title, grade, deadline, category, url, summary, why, kind="horizon"),
            encoding="utf-8",
        )
        written += 1

    dash = base / "_Horizons Dashboard.md"
    if not (args.add_only and dash.exists()):
        dash.write_text(dashboard("horizon", "Horizons"), encoding="utf-8")
