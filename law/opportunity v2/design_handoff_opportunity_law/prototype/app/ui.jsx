...
function CellBar({ frac, color = "var(--accent)" }) {
  return (
    <span className="cellbar">
      <span className="track"><span className="fill" style={{ width: `${Math.round(clamp(frac * 100))}%`, background: color, display: "block" }}></span></span>
  ...
  return (
    <Modal title="Methodology" onClose={onClose}>
      <div className="prose">
        ...
        <p className="mono">Prototype note: this demo runs on realistic sample data for 24 schools.
          The production app scores all 196.</p>
      </div>
    </Modal>);
